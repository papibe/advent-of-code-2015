...
func solve(wires map[string]Instruction, instructions []Instruction) int {
...
func solution(filename string) int {
	...
	a_value := solve(wires, instructions)

	// patch signal for "b"
	instruction := wires["b"]
	instruction.param1 = strconv.FormatInt(int64(a_value), 10)
	wires["b"] = instruction

	return solve(wires, instructions)
}

func main() {
	fmt.Println(solution("./input.txt")) // 2797
}
